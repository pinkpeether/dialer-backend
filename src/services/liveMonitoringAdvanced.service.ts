...
const REGION_BUCKETS: RegionBucket[] = [
  ...
  { key: '+968', label: 'Oman', lat: 21.4735, lng: 53.8478 },
...
const detectRegion = (phone?: string | null): RegionBucket => {
  ...
  const match = REGION_BUCKETS.sort((a, b) => b.key.length - a.key.length).find(bucket => normalized.startsWith(bucket.key))
...
const recommendDialRatio = (answerRate: number, readyAgents: number, activeCalls: number) => {
  if (readyAgents <= 0) return { recommendedRatio: 1, reason: 'No READY agents available. Keep ratio safe.', shouldAutoAdjust: false }
  const pressure = activeCalls / Math.max(readyAgents, 1)
  if (pressure >= 3) return { recommendedRatio: 1, reason: 'Active-call pressure is high. Reduce dialing pressure.', shouldAutoAdjust: true }
  if (answerRate >= 55) return { recommendedRatio: 1, reason: 'High answer rate. Keep ratio conservative to avoid abandoned calls.', shouldAutoAdjust: true }
  if (answerRate >= 35) return { recommendedRatio: 2, reason: 'Healthy answer rate. Moderate progressive/predictive pacing is safe.', shouldAutoAdjust: true }
  if (answerRate >= 20) return { recommendedRatio: 3, reason: 'Lower answer rate. Increase pacing carefully while monitoring abandoned calls.', shouldAutoAdjust: true }
  return { recommendedRatio: 4, reason: 'Low answer rate. Higher dialing ratio may be useful, but keep guardrails active.', shouldAutoAdjust: true }
}
...
const buildHourlyHeatmap = (calls: Array<{ startedAt: Date; status: string; disposition: string | null }>) => {
  const slots = Array.from({ length: 24 }).map((_, offset) => {
    ...
    return { hour: date.getHours(), label: `${String(date.getHours()).padStart(2, '0')}:00`, totalCalls: 0, answeredCalls: 0, completedCalls: 0, answerRate: 0, intensity: 0 }
  })
  ...
  return slots.map(slot => ({ ...slot, answerRate: calculateAnswerRate(slot.answeredCalls, slot.totalCalls), intensity: Math.round((slot.totalCalls / maxCalls) * 100) }))
}
...
export const getLiveMonitoringAdvancedViews = async (campaignId?: number, actor?: Scope.ScopeActor) => {
  ...
  const visibleAccountIds = !Scope.isPlatformActor(actor) ? await Scope.getActorAccountIds(actor) : null
  const last24Hours = getSinceDate(24)

  const liveCalls = await prisma.call.findMany({
    where: { ...callScope, ...campaignFilter, status: { in: [...ACTIVE_CALL_STATUSES] as never } },
    select: {
      id: true,
      status: true,
      disposition: true,
      ...
      remoteNumber: true,
      source: true,
      campaign: { select: { id: true, name: true, mode: true, dialingRatio: true, commercialAccount: { select: commercialAccountSelect } } },
  ...
  const recentCalls = await prisma.call.findMany({
    where: { ...callScope, ...campaignFilter, startedAt: { gte: last24Hours } },
    select: { id: true, status: true, disposition: true, startedAt: true, endedAt: true, duration: true, campaignId: true, agentId: true },
    orderBy: { startedAt: 'asc' },
    take: 2000,
  })

  const agents = await prisma.user.findMany({
    where: { ...userScope, role: 'AGENT', isActive: true },
    select: {
      id: true,
      name: true,
      email: true,
      ...
      commercialMemberships: {
        where: {
          status: 'ACTIVE',
          ...(visibleAccountIds ? { accountId: { in: visibleAccountIds.length ? visibleAccountIds : [-1] } } : {}),
        },
      ...
      calls: {
        where: { ...callScope, startedAt: { gte: last24Hours } },
        select: { id: true, status: true, disposition: true, startedAt: true, duration: true },
        orderBy: { startedAt: 'desc' },
        take: 50,
      },
    },
  ...
  const mapBuckets = liveCalls.reduce<Record<string, { key: string; label: string; lat: number; lng: number; activeCalls: number; ringingCalls: number; answeredCalls: number; numbers: string[] }>>((acc, call) => {
    const region = detectRegion(call.remoteNumber || call.contact?.phone)
    if (!acc[region.key]) acc[region.key] = { key: region.key, label: region.label, lat: region.lat, lng: region.lng, activeCalls: 0, ringingCalls: 0, answeredCalls: 0, numbers: [] }
  ...
  const liveAgentActivity = agents.map(agent => {
    ...
    const commercialAccounts = agent.commercialMemberships.map(item => item.account).filter(Boolean)
  ...
  return {
    ...
    liveCallMap: Object.values(mapBuckets).map(bucket => ({ ...bucket, sampleNumbers: bucket.numbers.slice(0, 5), numbers: undefined })),
    hourlyHeatmap: buildHourlyHeatmap(recentCalls),
    liveAgentActivity,
    simultaneousCalls: { totalActive: activeCalls, initiated: statusCounts.INITIATED || 0, ringing: statusCounts.RINGING || 0, answered: statusCounts.ANSWERED || 0 },
    answerRateTracker: { window: '24h', totalCalls: recentCalls.length, answeredCalls: answeredRecent, answerRate: answerRate24h, readyAgents, busyAgents, onlineAgents, ...ratioRecommendation },
    notes: [
