...
export const getAllContacts = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const { campaignId, status, search, page, limit, commercialAccountId } = req.query
    const result = await ContactService.getAllContacts({
      campaignId: campaignId ? Number(campaignId) : undefined,
      commercialAccountId: commercialAccountId ? Number(commercialAccountId) : undefined,
...
export const getContactCalls = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
...
export const updateContact = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const contact = await ContactService.updateContact(Number(req.params.id), req.body, req.user)
...
export const uploadCSV = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (isNaN(campaignId)) return sendError(res, 'Invalid campaign ID', 400)
...
export const addToDNC = async (req: Request, res: Response, next: NextFunction) => {
  try {
...
export const getContactStats = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const stats = await ContactService.getContactStats(campaignId ? Number(campaignId) : undefined, req.user)
